...
countsFile  <- "input/exon_deletion_norm_counts.txt.gz"
scoresFile1 <- "input/intronic_intronic_guides.tsv.gz"
scoresFile2 <- "input/other_guides.tsv.gz"
annoFile    <- "input/Knockout.exons.human.csv.gz"
...
### Value: data.frame with exon annotation
    ...
    geneLFC <- aggregate(counts$RPE1_logFC[counts$Data_Subset == "ExonDeletion_TKOv3"],
                         by=list(gene=counts$Target_gene[counts$Data_Subset == "ExonDeletion_TKOv3"]),
                         FUN=mean, na.rm=T)
...
exscr$n.Cas9 <- { # number of Cas9 guides in exon-deletion pairs
    ...
    x <- aggregate(counts$Cas9_Guide[sel], by=list(scores$Event[sel]),
...
exscr$n.Cpf1 <- { # number of Cpf1 guides in exon-deletion pairs
    ...
    x <- aggregate(counts$Cpf1_Guide[sel], by=list(scores$Event[sel]),
                   FUN=function(x) {length(unique(x))})
    out <- x$x[match(anno$EVENT, x[,1])]
    out[is.na(out)] <- 0
    out
...
exscr$exonDel.clean <- {     # Number of deletion guide pairs, neither of which is significant
    ...
    sel <- which(grepl("intronic",scores$Cas9_Guide_Type) & grepl("intronic",scores$Cpf1_Guide_Type) &
                 !(counts$Cas9_Guide %in% counts$Cas9_Guide[bad.Cas9]) &
                 !(counts$Cpf1_Guide %in% counts$Cpf1_Guide[bad.Cpf1]))
